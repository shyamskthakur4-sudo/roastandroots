...
export function HomePage({ onAdd }) {
  return (
    <>
      ...
      <main>
        <section className="relative bg-[#f4f7f2] py-28 lg:py-40">
          <div className="mx-auto max-w-[1440px] px-5 lg:px-9">
            <div className="grid gap-14 lg:grid-cols-[.55fr_1.45fr] lg:items-end">
              <div>
                <SectionTitle
                  eyebrow="The collection"
                  title="Four favourites. No filler."
                  copy="Start with the products people reach for most."
                />
                <Link to="/shop" className="mt-8 inline-flex items-center gap-2 text-sm font-semibold">
                  View all products <ArrowRight size={17} />
                </Link>
              </div>

              <div className="grid grid-cols-2 gap-x-6 gap-y-20 lg:grid-cols-4 [perspective:1600px]">
                {products.slice(0, 4).map((product, index) => (
                  <ScrollDepth
                    key={product.id}
                    lift={index % 2 ? 85 : 35}
                    rotate={index % 2 ? -2.2 : 2.2}
                  >
                    <ProductCard product={product} onAdd={onAdd} index={index} />
                  </ScrollDepth>
                ))}
              </div>
            </div>
        ...
        <section className="relative overflow-hidden bg-[#152019] py-0 text-white">
          <div className="mx-auto grid min-h-[92dvh] max-w-[1440px] items-center gap-16 px-5 py-24 lg:grid-cols-[.7fr_1.3fr] lg:px-9 lg:py-0">
            <div className="relative z-10">
              <Reveal>
                <p className="text-[10px] font-semibold uppercase tracking-[.24em] text-[#d8b56d]">How we roast</p>
                <h2 className="mt-5 text-5xl font-semibold leading-[.9] tracking-[-.07em] md:text-7xl">
                  The ingredient
                  <br />
                  stays in charge.
                </h2>
                <p className="mt-7 max-w-md text-base leading-7 text-white/62">
                  Simple seasoning, careful roasting and a focus on the natural crunch.
                </p>
              </Reveal>
            </div>

            <ScrollDepth lift={110} rotate={2.8} scale={1.05}>
              <div className="relative mx-auto aspect-[1.08] w-full max-w-[760px] [transform-style:preserve-3d]">
                <motion.div
                  style={{ transform: "translateZ(30px)" }}
                  className="absolute left-[4%] top-[8%] h-[74%] w-[64%] overflow-hidden rounded-[32px] shadow-[0_40px_110px_rgba(0,0,0,.3)]"
                >
                  <img
                ...
                <motion.div
                  ...
                  className="absolute bottom-[2%] right-[2%] h-[58%] w-[51%] overflow-hidden rounded-[32px] border border-white/15 shadow-[0_40px_100px_rgba(0,0,0,.25)]"
                >
                  <img
                ...
                <div className="absolute right-[36%] top-[35%] z-10 rounded-full border border-[#d9b76b]/45 bg-[#d9b76b]/10 px-5 py-3 text-[10px] font-bold uppercase tracking-[.2em] backdrop-blur-md">
                  Small-batch roast
                </div>
              </div>
            </ScrollDepth>
        ...
        <section className="relative overflow-hidden bg-[#0f1813] py-28 text-white lg:py-40">
          <div className="mx-auto max-w-[1440px] px-5 lg:px-9">
            <div className="flex items-end justify-between gap-8">
              <SectionTitle
                ...
                copy="Pure, sweet, savoury or made to gift."
              />
              <span className="hidden text-[10px] uppercase tracking-[.22em] text-white/35 md:block">
                Scroll through the collection
              </span>
            </div>

            <div className="mt-16 grid gap-8 md:grid-cols-2 [perspective:1800px]">
              {collections.map((item, index) => (
                <ScrollDepth
                  key={item.slug}
                  lift={index % 2 ? 100 : 145}
                  rotate={index % 2 ? 2.6 : -2.6}
                  scale={1.035}
                >
                  <TiltCard className={index === 0 ? "md:row-span-2" : ""}>
                    <Link
                      to={`/collection/${item.slug}`}
                      className={`group relative block overflow-hidden rounded-[32px] border border-white/10 ${index === 0 ? "min-h-[700px]" : "min-h-[330px]"}`}
                    >
                      <Parallax src={item.image} alt={item.title} scale={1.15} y={40} />
                      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(0,0,0,.02)_30%,rgba(0,0,0,.84)_100%)]" />
                      <div className="relative flex h-full min-h-[330px] flex-col justify-end p-7 lg:p-10">
                        <p className="text-[9px] font-bold uppercase tracking-[.24em] text-[#e0b969]">
                          {item.eyebrow}
                        </p>
                        <h3 className="mt-3 max-w-xl text-4xl font-semibold leading-[.92] tracking-[-.065em] md:text-6xl">
                          {item.title}
                        </h3>
                        <span className="mt-6 inline-flex items-center gap-2 text-sm font-semibold">
                          Explore <ArrowRight size={17} className="transition-transform group-hover:translate-x-1" />
                        </span>
                      </div>
                    </Link>
                  </TiltCard>
        ...
        <section className="bg-[#f4f7f2] py-28 lg:py-40">
          <div className="mx-auto max-w-[1440px] px-5 lg:px-9">
            <SectionTitle
              eyebrow="From the people who snack"
              title="Good enough to share."
              copy="A few words from recent customers."
            />
            <div className="mt-14 grid gap-8 md:grid-cols-3 [perspective:1500px]">
              {reviews.map((review, index) => (
                <ScrollDepth key={review.name} lift={45 + index * 15} rotate={index === 1 ? 2.2 : -1.6}>
                  <TiltCard className="h-full">
                    <blockquote className="flex min-h-[300px] h-full flex-col justify-between rounded-[28px] border border-[#d7ddd6] bg-white p-7">
                      <div>
                        <Quotes size={28} className="text-[#d4aa58]" />
                        <p className="mt-6 text-xl font-medium leading-8 tracking-[-.03em]">“{review.quote}”</p>
                      </div>
                      <footer className="mt-10 border-t border-[#e2e7e1] pt-4">
                        <span className="text-sm font-semibold">{review.name}</span>
                        <span className="ml-2 text-[10px] uppercase tracking-[.16em] text-[#7a847c]">{review.role}</span>
                      </footer>
                    </blockquote>
                  </TiltCard>
                </ScrollDepth>
