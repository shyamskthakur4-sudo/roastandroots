...
function Hero3D() {
  ...
  const productRotate = useTransform(scrollYProgress, [0, 1], [0, reduce ? 0 : -4]);
  const heroProduct = products.find((product) => product.id === "honey") ?? products[0];
  return (
    <section ref={ref} className="relative min-h-[112dvh] overflow-hidden bg-[#0d1712] text-white">
      ...
      <motion.div style={{ y: productY, rotate: productRotate }} className="absolute bottom-[10%] right-[5%] z-20 hidden w-[min(25vw,330px)] md:block">
        <div className="overflow-hidden rounded-[30px] border border-white/20 bg-black/20 p-2 shadow-[0_35px_90px_rgba(0,0,0,.28)]">
          <div className="relative overflow-hidden rounded-[24px] bg-[#1b241f]">
            <img
              src={heroProduct.gallery?.[1] ?? heroProduct.image}
              alt={heroProduct.name}
              className="aspect-[.78] w-full object-cover"
              onError={(event) => { event.currentTarget.src = "https://images.unsplash.com/photo-1590080875515-8a3a8dc5735e?auto=format&fit=crop&w=1200&q=90"; }}
            />
          </div>
          <div className="absolute inset-x-5 bottom-5 rounded-[18px] border border-white/15 bg-[#101814]/78 p-4 backdrop-blur-xl">
            <div className="text-[9px] uppercase tracking-[.2em] text-white/55">Signature flavour</div>
            <div className="mt-1 text-lg font-semibold">{heroProduct.name}</div>
          </div>
        </div>
      </motion.div>
      <motion.div style={{ y: copyY, scale: copyScale }} className="relative mx-auto flex min-h-[112dvh] max-w-[1440px] items-center px-5 pb-24 pt-28 lg:px-9">
        <div className="max-w-[820px] pr-0 lg:pr-8">
          ...
          <Reveal delay={0.08}><h1 className="mt-6 max-w-[820px] text-[clamp(3.2rem,6.2vw,6.8rem)] font-semibold leading-[0.9] tracking-[-0.07em]">Rooted in purity.<br /><span className="text-[#deb76a]">Roasted to perfection.</span></h1></Reveal>
          <Reveal delay={0.16}><p className="mt-7 max-w-[690px] text-base leading-7 text-white/68 md:text-lg">Hand-roasted premium dry fruits in small batches. No shortcuts. Just honest taste, thoughtful flavours and a better snack ritual.</p></Reveal>
